...
const MOCK_TECHNICIANS = Array.from({ length: 30 }, (_, i) => ({
    id: `tech-${i + 1}`,
    name: `担当者 ${String.fromCharCode(65 + (i % 26))}${i + 1}`,
    email: `tech${i + 1}@example.com`,
    area: i % 3 === 0 ? '渋谷・新宿' : i % 3 === 1 ? '世田谷・目黒' : '港・品川',
    current_active_cases: i % 5,
    company_name: '株式会社デモメンテ',
    phone: `000-1000-${String(i + 1).padStart(4, '0')}`,
    detergent: 'あり',
    machine: 'あり',
}));
